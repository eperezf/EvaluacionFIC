...
<form method="POST" action="{{ route('postAgregar') }}" id="agregar-tutoria">
  <div class="row">
    <section id="agregar" name="Agregar Tutoria" class="col-6">
      @csrf
      <h3>Agregar una tutoría</h3>
      <div id="nombre" class="form-group row">
        <label for="nombre-input" class="col-sm-3 col-form-label">Tutoría</label>
        <div class="col-sm-9">
        <input name="nombre" class="form-control col-sm-10" placeholder="Nombre de la tutoría" type="text" id="nombre-input" value="{{ old('nombre') }}">
        </div>
      </div>
